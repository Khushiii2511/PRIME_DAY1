...
const showUI = (list) => {
  ...
  if (list.length === 0) {
    root.innerHTML = '<div class="no-results">No videos found matching your search.</div>';
    return;
  }
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // If we have a search query, update the search input field
  if (searchQuery) {
    searchInput.value = searchQuery;
    
    // Filter videos based on search term
    const filteredVideos = filterVideos(dummydata, searchQuery);
